...
const audioJogo = new Audio('../audios/audioJogo.mp3')
...
const velocidadeNave = 30
const velocidadeTiro = 30
const velocidadeNaveInimigas = 7.5
...
const criaTiros = (posicaoLeftTiro, posicaoTopTiro) => {
    ...
    tiro.style.height = '7px'
    tiro.style.borderRadius = '5px'
...
const audioTiros = () => {
    ...
    audioDoTiro.setAttribute('src', '../audios/audioTiro1.mp3')
    audioDoTiro.play()
    cenario.appendChild(audioDoTiro)
    audioDoTiro.addEventListener('ended', () => { audioDoTiro.remove() })
}
...
const moveNaveInimigas = () => {
    ...
    for (let i = 0; i < naveInimigas.length; i++) {
        ...
        if (naveInimigas[i]) {
            ...
            if (posicaoTopNaveInimiga > alturaCenario - (alturaNave / 2.5)) {
                vidaAtual -= 5
...
const naveInimigaDestruida = (posicaoLeftNaveInimiga, posicaoTopNaveInimiga) => {
    ...
    naveInimigaDestruida.style.position = 'center'
    ...
    audioExplosoes()
    setTimeout(
        () => { cenario.removeChild(naveInimigaDestruida) }, 1000);
}
...
const explosaoNaveInimigaDestruida = (posicaoLeftNaveInimiga) => {
    ...
    explosaoNaveInimiga.style.position = 'center'
    ...
    audioExplosoes()
    setTimeout(
        () => { cenario.removeChild(explosaoNaveInimiga) }, 500);
}
...
const audioExplosoes = () => {
    ...
    audioExplosaoNaveInimiga.addEventListener('ended', () => { audioExplosaoNaveInimiga.remove() })
}
...
const gameOver = () => {
    ...
    perdeu.style.backgroundColor = 'rgb(224, 67, 67)'
    perdeu.style.top = '50%'
    perdeu.style.left = '50%'
    perdeu.style.fontSize = '20px'
    perdeu.style.padding = '10px 20px'
    perdeu.style.color = 'black'
    perdeu.style.borderRadius = '12px'
    perdeu.style.letf = '50%'
    perdeu.style.top = '50%'
    ...
    navesInimigas.forEach(
        (inimigos) => { inimigos.remove() }
    )

    const todosTiros = document.querySelectorAll('.tiro')
    todosTiros.forEach(
        (tiro) => { cenario.removeChild(tiro) }
    )
}
